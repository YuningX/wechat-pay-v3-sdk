...
var WechatPayV3Base = class {
  ...
  init() {
    ...
    this.request.interceptors.request.use(async (config) => {
      ...
      if (config.headers?.Authorization === void 0) {
   ...
   */
  sha256WithRSA(data) {
    return crypto2.createSign("RSA-SHA256").update(data).sign(this.privateKey, "base64");
  }
  /**
   * 平台证书公钥验签
   * @param serial 证书序列号
  ...
  sha256WithRsaVerify(serial, signature, data) {
    ...
    return crypto2.createVerify("RSA-SHA256").update(data).verify(cert.publicKey, signature, "base64");
  }
  /**
   * 解密平台响应
   * @param nonce 随机字符串
